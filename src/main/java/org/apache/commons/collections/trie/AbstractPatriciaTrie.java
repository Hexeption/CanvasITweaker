...
abstract class AbstractPatriciaTrie<K, V> extends AbstractBitwiseTrie<K, V> {
    ...
    /**
     * Constructs a new {@link org.apache.commons.collections.Trie Trie} using the given
     * {@link KeyAnalyzer} and initializes the {@link org.apache.commons.collections.Trie Trie}
    ...
    public Entry<K, V> select(final K key) {
        final int lengthInBits = lengthInBits(key);
        final Reference<Entry<K, V>> reference = new Reference<Entry<K,V>>();
    ...
    public K selectKey(final K key) {
        final Entry<K, V> entry = select(key);
        if (entry == null) {
            return null;
        }
        return entry.getKey();
    ...
    private boolean selectR(final TrieEntry<K, V> h, final int bitIndex,
                            ...
                            final Reference<Entry<K, V>> reference) {
    ...
    private class EntrySet extends AbstractSet<Entry<K,V>> {
        ...
        public boolean contains(final Object o) {
            ...
            final TrieEntry<K,V> candidate = getEntry(((Entry<?, ?>)o).getKey());
   ...
   private class RangeEntryMap extends RangeMap {
    ...
    private class PrefixRangeMap extends RangeMap {
        ...
        private int fixup() {
            ...
            if (size == - 1 || AbstractPatriciaTrie.this.modCount != expectedModCount) {
                final Iterator<Entry<K, V>> it = super.entrySet().iterator();
                size = 0;
    ...
    private final class PrefixRangeEntrySet extends RangeEntrySet {
        ...
        public Iterator<Entry<K,V>> iterator() {
            ...
            if (prefixStart == null) {
                final Set<Entry<K,V>> empty = Collections.emptySet();
