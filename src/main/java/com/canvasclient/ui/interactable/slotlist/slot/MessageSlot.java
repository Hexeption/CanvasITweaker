...
import com.canvasclient.ui.popup.UIPopupMessageReply;
...
public class MessageSlot extends Slot {
	...
	public void doubleClick(int mx, int my) {
		((UIInGameMenu) Canvas.getCanvas().getUIHandler().getCurrentUI()).changePopup(new UIPopupMessageReply(Canvas.getCanvas().getUIHandler().getCurrentUI(), MessageManager.getMessage(index).minecraftName, MessageManager.getMessage(index).message));
	}
