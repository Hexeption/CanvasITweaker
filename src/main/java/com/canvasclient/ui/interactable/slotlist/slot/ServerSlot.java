	...
	public void render(float opacity) {
		...
		if (ServerManager.getServer(index) != null) {
			...
			if (ServerManager.getServer(index).serverData.populationInfo != null) {
				...
				if (parts.length == 2) {
					GuiHelper.drawStringFromTTF(FontHandler.ROUNDED_BOLD.get(12), x+width-32, y+(height/2), parts[0]+"\u00a78/"+parts[1], Utilities.reAlpha(Canvas.getCanvas().getColorSchemeHandler().getCurrentBackground(), 1f*opacity), Align.RIGHT, Align.CENTER);
					GuiHelper.drawTexturedRectangle(x+width-24, y+(height/2)-4, 8, 8, ResourceHelper.iconUser[0], Utilities.reAlpha(Canvas.getCanvas().getColorSchemeHandler().getCurrentBackground(), 1f*opacity));
				}
			}
			if (index > 0) {
