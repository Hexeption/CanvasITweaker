...
import com.canvasclient.ui.interactable.PasswordBoxAlt;
...
public class UICreatePassport extends UI {
	...
	public void init() {
		...
		title = "Create a Canvas Passport";
		this.canvasHead = ResourceHelper.downloadTexture(String.format(APIHelper.PLAYER_HEAD, Minecraft.getMinecraft().getSession().getUsername(), "512"));
		interactables.clear();
		int i = 0;
		...
		interactables.add(new PasswordBoxAlt(this, password, width / 2.1F + FontHandler.ROUNDED_LIGHT.get(25).getWidth("MyCanvas Password:  "), height / 1.7F, 290, FontHandler.ROUNDED_LIGHT.get(25).getHeight(), FontHandler.ROUNDED_BOLD.get(25), 0xFF252525, 0xFFD7D7D7, Align.LEFT, Align.CENTER, false));
	...
	public void render(float opacity) {
		...
		Texture head = this.canvasHead.getTexture();
		...
		GuiHelper.drawStringFromTTF(FontHandler.ROUNDED_LIGHT.get(25), width / 2.1F, height / 1.7F, "MyCanvas Password:", 0xFF464646);
				...
				try { 
					if (clientID.length() > 0) {
						...
						if (clientDetails.length == 12) {
							Canvas.getCanvas().setUserControl(new UserControl(clientDetails[0], clientDetails[1], clientDetails[2], clientDetails[3], clientDetails[4], clientDetails[5], clientDetails[6], clientDetails[7], clientDetails[8]));
							return true;
						} else {
							Canvas.getCanvas().setUserControl(null);
							return true;
						}
					} else {
						Canvas.getCanvas().setUserControl(null);
					...
					Canvas.getCanvas().setUserControl(new UserControl(clientID, "User", "", "false", "", "verified", "", "30", "4"));
					return true;
				}
			} else {
				message = response[1];
	...
	public void handleInteraction(Interactable i) {
		...
		case 0:
			Canvas.getCanvas().getUIHandler().changeUI(new UILogin(this));
			break;
		case 5 :
			if (submit())
				Canvas.getCanvas().getUIHandler().changeUI(new UIColorChooser(this));
		}
	}
}
