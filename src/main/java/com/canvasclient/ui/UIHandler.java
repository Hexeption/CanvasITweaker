...
public class UIHandler {
		...
		EventManager.get().register(new Listener<EventKeyPressed>(){
			...
			public void call(EventKeyPressed event) {
				...
				if (event.key == Keyboard.KEY_M) {
					...
					Canvas.getCanvas().getUIHandler().changeUI(new UIMusic(true));
					return;
				}
			}
		});
					...
					if (Keyboard.isCreated()) {
						...
						while (Keyboard.next()) {
							if (Keyboard.getEventKeyState()) {
								Canvas.getCanvas().getUIHandler().onKeyPressed(Keyboard.getEventKey());
								Canvas.getCanvas().getUIHandler().onKeyTyped(Keyboard.getEventKey(), Keyboard.getEventCharacter());
							} else {
								Canvas.getCanvas().getUIHandler().onKeyRelease(Keyboard.getEventKey());
							}
						}
						...
						while (Mouse.next()) {
							if (Mouse.getEventButton() == -1) {
								if (Mouse.getEventDWheel() != 0)
									Canvas.getCanvas().getUIHandler().onMouseScroll(Mouse.getEventDWheel());
								mouse[0] = Mouse.getEventX();
								mouse[1] = Display.getHeight()-Mouse.getEventY();
							} else {
								if (Mouse.getEventButtonState())
									Canvas.getCanvas().getUIHandler().onMouseClick(mouse[0], mouse[1], Mouse.getEventButton());
								else
									Canvas.getCanvas().getUIHandler().onMouseRelease(mouse[0], mouse[1], Mouse.getEventButton());
							}
						}
					}
	...
	public void render2D() {
		...
		Display.setTitle(Canvas.getCanvas().getName() + " " + Canvas.getCanvas().getVersion() + " (" + Minecraft.getMinecraft().getDebugFPS() + ")");
