...
public class UIPopupPlaylistEdit extends UI {
	...
	public UIPopupPlaylistEdit(UI parent, Playlist playlist) {
		super(parent);
	...
	public void init() {
		...
		interactables.add(new Button(this, "Edit", fX+(fW/2)-136, fY+(fH/2)-48, 120, 32, FontHandler.ROUNDED_BOLD.get(14), KiLO.getKiLO().getColorSchemeHandler().getCurrentBackground(), ResourceHelper.iconEdit[1], 16, Align.LEFT, Align.CENTER));
		...
		if (playlist != null) {
			((TextBox)interactables.get(2)).text = playlist.name;
		}
	}
	...
	public void update(int mx, int my) {
		...
		((Button)interactables.get(1)).text = checking?(String)null:"Edit";
	}
	...
	public void submit() {
		...
		if (name != null && name.length() > 0) {
			...
			new Thread() {
				@Override
				public void run() {
					if (APIHelper.playlistRename(KiLO.getKiLO().getUserControl().clientID, playlist.id, name)) {
