	...
	public void submit() {
		...
		if (name != null && name.length() > 0 && msg != null && msg.length() > 0) {
			...
			new Thread() {
				@Override
				public void run() {
					if (APIHelper.sendMessage(KiLO.getKiLO().getUserControl().clientID, name, msg)) {
						((UIInGameMenu)parent).changePopup(null);
					} else {
						checking = false;
	...
	public void render(float opacity) {
		...
		if (checking) {
			...
			if (KiLO.getKiLO().getUserControl() != null && KiLO.getKiLO().getUserControl().kiloHead != null) {
				GuiHelper.drawTexturedRectangle(fX+(fW/2)-16-48, fY+(fH/2)-16-48, 48, 48, KiLO.getKiLO().getUserControl().kiloHead.getTexture(), Utilities.reAlpha(ColorHelper.WHITE.getColorCode(), 1f*opacity));
			}
		}
