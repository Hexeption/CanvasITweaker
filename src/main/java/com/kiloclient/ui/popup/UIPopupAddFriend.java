...
public class UIPopupAddFriend extends UI {
	...
	public void init() {
		...
		interactables.add(new Button(this, "Request", fX+(fW/2)-136, fY+(fH/2)-48, 120, 32, FontHandler.ROUNDED_BOLD.get(14), KiLO.getKiLO().getColorSchemeHandler().getCurrentBackground(), ResourceHelper.iconSubmit[1], 16, Align.LEFT, Align.CENTER));
	...
	public void submit() {
		...
		if (name != null && name.length() > 0) {
			...
			new Thread() {
				@Override
				public void run() {
					if (APIHelper.friendAdd(KiLO.getKiLO().getUserControl().clientID, name, msg)) {
						if (parent instanceof UIInGameMenuFriend)
							((UIInGameMenuFriend)parent).changePopup(null);
						else
							((UIInGameMenu)parent).changePopup(null);
