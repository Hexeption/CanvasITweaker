...
public class UIPopupInviteFriends extends UI {
	...
	public void init() {
		...
		interactables.add(new Button(this, "Invite", fX+(fW/2)-136, fY+(fH/2)-48, 120, 32, FontHandler.ROUNDED_BOLD.get(14), KiLO.getKiLO().getColorSchemeHandler().getCurrentBackground(), ResourceHelper.iconSubmit[1], 16, Align.LEFT, Align.CENTER));
		interactables.add(new TextBoxAlt(this, "Enter ingame name...", fX-(fW/2)+48+fH-80, fY-(fH/2)+64, fW-(48+fH-80)-32, 32, FontHandler.ROUNDED_BOLD.get(20), -1, Align.LEFT, Align.CENTER));
		interactables.add(new TextBoxAlt(this, "Invite message...", fX-(fW/2)+48+fH-80, fY-(fH/2)+64+32+16, fW-(48+fH-80)-32, 32, FontHandler.ROUNDED_BOLD.get(20), -1, Align.LEFT, Align.CENTER));
	}
	...
	public void update(int mx, int my) {
		...
		((Button)interactables.get(1)).text = checking?(String)null:"Invite";
	}
	...
	public void submit() {
		...
		if (name != null && name.length() > 0) {
			...
			new Thread() {
				@Override
				public void run() {
					if (APIHelper.sendServerInvite(KiLO.getKiLO().getUserControl().clientID, name, mc.getCurrentServerData().serverIP, msg)) {
						((UIInGameMenu)parent).changePopup(null);
					} else {
						checking = false;
	...
	public void render(float opacity) {
		...
		if (KiLO.getKiLO().getUserControl() != null && KiLO.getKiLO().getUserControl().kiloHead != null) {
			GuiHelper.drawTexturedRectangle(fX-(fW/2)+16, fY-(fH/2)+64, fH-80, fH-80, KiLO.getKiLO().getUserControl().kiloHead.getTexture(), Utilities.reAlpha(ColorHelper.WHITE.getColorCode(), 1f*opacity));
		}
		
		GuiHelper.drawStringFromTTF(FontHandler.ROUNDED_BOLD.get(12), fX-(fW/2)+48+fH-80, fY+(fH/2)-80, "They will join \u00a73"+mc.getCurrentServerData().serverIP, Utilities.reAlpha(ColorHelper.WHITE.getColorCode(), 1f*opacity));
		...
		if (((TextBox)interactables.get(2)).getText().length() > 0 && (interactables.get(2).active)) {
			...
			if (!acnames.isEmpty()) {
				...
				for(int i = 0; i < acnames.size(); i++) {
					GuiHelper.drawStringFromTTF(FontHandler.ROUNDED_BOLD.get(14), xx+2, yy+hh+((i)* FontHandler.ROUNDED_BOLD.get(14).getHeight()), acnames.get(i), Utilities.reAlpha(acSelect == i?KiLO.getKiLO().getColorSchemeHandler().getCurrentBackground():ColorHelper.WHITE.getColorCode(), 1f*opacity));
				}
			} else {
				acSelect = -1;
			}
