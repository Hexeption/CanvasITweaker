...
public class UIPopupPlaylistCreate extends UI {
	...
	public void init() {
		...
		interactables.add(new Button(this, "Create", fX+(fW/2)-136, fY+(fH/2)-48, 120, 32, FontHandler.ROUNDED_BOLD.get(14), KiLO.getKiLO().getColorSchemeHandler().getCurrentBackground(), ResourceHelper.iconAdd[1], 16, Align.LEFT, Align.CENTER));
	...
	public void submit() {
		...
		if (name != null && name.length() > 0) {
			...
			new Thread() {
				@Override
				public void run() {
					if (APIHelper.playlistAdd(KiLO.getKiLO().getUserControl().clientID, name)) {
