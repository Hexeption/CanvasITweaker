...
public class UIPopupPlaylistDelete extends UI {
	...
	public void init() {
		...
		interactables.add(new Button(this, "Yes", fX-(fW/2)+24, fY+(fH/2)-24-48, (fW/2)-24-12, 48, FontHandler.ROUNDED_BOLD.get(20), KiLO.getKiLO().getColorSchemeHandler().getCurrentBackground(), null, 0));
		...
		case 1:
			if (!deleting) {
				...
				new Thread() {
					@Override
					public void run() {
						if (APIHelper.playlistRemove(KiLO.getKiLO().getUserControl().clientID, playlist.id)) {
							((UIMusic)parent).changePopup(null);
							KiLO.getKiLO().getMusicHandler().getPlaylists().remove(playlist);
						} else {
							invalid = true;
