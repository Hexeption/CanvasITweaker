...
import com.kiloclient.ui.interactable.slotlist.ChatList;
...
public class UIChat extends UI {
	...
	public static void updateHistory(int mx, int my) {
		...
		if (!(KiLO.getKiLO().getUIHandler().getCurrentUI() instanceof UIChat) && !(KiLO.getKiLO().getUIHandler().getUITo() instanceof UIChat)) {
			dY+= (0-dY)/2f;
			sl.scrollTo = 0;
			float chatSize = 0;
			...
			if (KiLO.getKiLO().getUIHandler().getCurrentUI() instanceof UIChat) {
				dY+= ((-pad-2-(((UIChat)KiLO.getKiLO().getUIHandler().getCurrentUI()).interactables.get(0)).height)-dY)/2f;
			} else {
				dY+= ((-pad-2-(((UIChat)KiLO.getKiLO().getUIHandler().getUITo()).interactables.get(0)).height)-dY)/2f;
			}
		}
	...
	public void render(float opacity) {
		if (hoverComponent != null) {
			ChatManager.handleComponentHover(hoverComponent, KiLO.getKiLO().getUIHandler().mouse[0], KiLO.getKiLO().getUIHandler().mouse[1]);
		}
		super.render(opacity);
	}
