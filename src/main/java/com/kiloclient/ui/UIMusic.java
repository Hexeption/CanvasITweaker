...
import com.kiloclient.ui.interactable.ButtonMusic;
...
import com.kiloclient.ui.interactable.VolumeSlider;
...
import com.kiloclient.ui.interactable.slotlist.slot.PlaylistSlot;
import com.kiloclient.ui.interactable.slotlist.slot.Slot;
import com.kiloclient.ui.interactable.slotlist.slot.SongSlot;
import com.kiloclient.ui.popup.UIPopupPlaylistCreate;
...
public class UIMusic extends UI {
	...
	public void init() {
		...
		interactables.add(new IconButton(this, fX-(fW/2)+200-16, fY-(fH/2)+132+32+4, 8, 8, KiLO.getKiLO().getColorSchemeHandler().getCurrentForeground(), ResourceHelper.iconPlus[0]));
		...
		interactables.add(new IconButton(this, fX-(fW/2)+80+150, fY+(fH/2)-24-16, 32, 32, KiLO.getKiLO().getColorSchemeHandler().getCurrentForeground(), ResourceHelper.iconPlay[3]));
	...
	public void update(int mx, int my) {
		...
		for(Slot s : psl.slots) {
			if (KiLO.getKiLO().getMusicHandler().getPlaylists().size() == 0)
				continue;
			if (s.active) {
				activePlaylist = KiLO.getKiLO().getMusicHandler().getPlaylists().get(((PlaylistSlot)s).index);
				break;
			}
		}
		...
		int icon = (int)(KiLO.getKiLO().getMusicHandler().getVolume()*3f/100f);
		...
		if (psl.slots.size() != KiLO.getKiLO().getMusicHandler().getPlaylists().size()) {
			psl.slots.clear();
			int i = 0;
			for(Playlist f : KiLO.getKiLO().getMusicHandler().getPlaylists()) {
				psl.slots.add(new PlaylistSlot(psl, KiLO.getKiLO().getMusicHandler().getPlaylists().indexOf(f), psl.x, psl.y, psl.w, 24, 0, i*24));
				i++;
			}
		}
		...
		case search:
			if (loading && searchTags.length() > 0) {
				...
				new Thread() {
					@Override
					public void run() {
						List<Song> s = APIHelper.getSearchSongs(KiLO.getKiLO().getUserControl().clientID, displayTags.toUpperCase());
						if (s.isEmpty()) {
							ssl.slots.clear();
							loading = false;
		...
		case charts:
			if (loading && loadCharts) {
				...
				new Thread() {
					@Override
					public void run() {
						List<Song> s = APIHelper.getSongCharts(KiLO.getKiLO().getUserControl().clientID);
						if (s.isEmpty()) {
							ssl.slots.clear();
							loading = false;
							mode = Mode.home;
		...
		case starred:
			if (loading && loadStars) {
				...
				new Thread() {
					@Override
					public void run() {
						List<Song> s = APIHelper.getSongStars(KiLO.getKiLO().getUserControl().clientID);
						if (s.isEmpty()) {
							ssl.slots.clear();
							loading = false;
							mode = Mode.home;
		...
		if (KiLO.getKiLO().getMusicHandler().getMusicPlayer() != null && KiLO.getKiLO().getMusicHandler().isPlaying()) {
		...
		if (KiLO.getKiLO().getMusicHandler().getMusicPlayer() == null || KiLO.getKiLO().getMusicHandler().getCurrentSong() == null || KiLO.getKiLO().getMusicHandler().getCurrentSongList() == null) {
		...
		if (KiLO.getKiLO().getMusicHandler().getCurrentSong() == null && !KiLO.getKiLO().getMusicHandler().isPlaying() && KiLO.getKiLO().getMusicHandler().getMusicPlayer() == null) {
	...
	public void handleInteraction(final Interactable i) {
		...
		case 7:
			...
			break;
		case 8:
			if (KiLO.getKiLO().getMusicHandler().getCurrentSong() != null && KiLO.getKiLO().getMusicHandler().isPlaying()) {
				KiLO.getKiLO().getMusicHandler().pauseCurrentSong();
			} else if (KiLO.getKiLO().getMusicHandler().getCurrentSong() != null) {
	...
	public void setVolume(int volume) {
		KiLO.getKiLO().getMusicHandler().setVolume(volume);
	}
	...
	public void render(float opacity) {
		...
		GuiHelper.drawRectangle(fX-(fW/2)+12, fY-(fH/2)+48+12+24+24+12, fX-(fW/2)+200-12, fY-(fH/2)+48+12+24+24+13, Utilities.reAlpha(KiLO.getKiLO().getColorSchemeHandler().getCurrentBackground(), 0.5f*opacity));
		...
		if (KiLO.getKiLO().getMusicHandler().getMusicPlayer() != null && KiLO.getKiLO().getMusicHandler().getCurrentSong() != null) {
			Song song = KiLO.getKiLO().getMusicHandler().getCurrentSong();
			if (song != null) {

				int pos = KiLO.getKiLO().getMusicHandler().getMusicPlayer().getPosition();
				...
				GuiHelper.drawRectangle(fX-(fW/2)+80+150+48+24+38+ FontHandler.STANDARD.get(12).getWidth(timeFormat)+3, fY+(fH/2)-24-3, fX-(fW/2)+80+150+48+24+38+ FontHandler.STANDARD.get(12).getWidth(timeFormat)+3+(barWidth*barPos), fY+(fH/2)-24+3, Utilities.reAlpha(KiLO.getKiLO().getColorSchemeHandler().getCurrentForeground(), 1f*opacity));
				GuiHelper.drawCircle(fX-(fW/2)+80+150+48+24+38+ FontHandler.STANDARD.get(12).getWidth(timeFormat)+3, fY+(fH/2)-24, 3, Utilities.reAlpha(KiLO.getKiLO().getColorSchemeHandler().getCurrentForeground(), 1f*opacity));
				GuiHelper.drawCircle(fX-(fW/2)+80+150+48+24+38+ FontHandler.STANDARD.get(12).getWidth(timeFormat)+3+(barWidth*barPos), fY+(fH/2)-24, 3, Utilities.reAlpha(KiLO.getKiLO().getColorSchemeHandler().getCurrentForeground(), 1f*opacity));
				...
				GuiHelper.drawStringFromTTF(FontHandler.ROUNDED_BOLD.get(12), fX-(fW/2)+64, fY+(fH/2)-24, title, Utilities.reAlpha(KiLO.getKiLO().getColorSchemeHandler().getCurrentForeground(), 1f*opacity), Align.LEFT, Align.CENTER);
			} else {
				GuiHelper.drawLoaderAnimation(fX-(fW/2)+24, fY+(fH/2)-24, 8, Utilities.reAlpha(ColorHelper.WHITE.getColorCode(), 1f*opacity));
			}
		}
		...
		case charts:
			if (!loading) {
				...
				GuiHelper.drawStringFromTTF(FontHandler.ROUNDED_BOLD.get(16), fX-(fW/2)+200+24, fY-(fH/2)+48+24+4, "Charts", Utilities.reAlpha(KiLO.getKiLO().getColorSchemeHandler().getCurrentForeground(), 1f*opacity), Align.LEFT, Align.CENTER);
		...
		case playlist:
			if (activePlaylist != null) {
				...
				GuiHelper.drawStringFromTTF(FontHandler.ROUNDED_BOLD.get(16), fX-(fW/2)+200+24, fY-(fH/2)+48+24+5, title, Utilities.reAlpha(KiLO.getKiLO().getColorSchemeHandler().getCurrentForeground(), 1f*opacity), Align.LEFT, Align.CENTER);
				GuiHelper.startClip(fX-(fW/2)+224, fY-(fH/2)+48+24+32, fX+(fW/2)-24+ssl.sbw, fY+(fH/2)-48-24);
				ssl.render(opacity);
				GuiHelper.endClip();
			}
		...
		case searchEmpty:
			...
			GuiHelper.drawStringFromTTF(FontHandler.ROUNDED_BOLD.get(16), fX-(fW/2)+200+24, fY-(fH/2)+48+24+16, "No results found", Utilities.reAlpha(KiLO.getKiLO().getColorSchemeHandler().getCurrentForeground(), 1f*opacity), Align.LEFT, Align.CENTER);
			break;
		case starred:
			if (!loading) {
				GuiHelper.drawRectangle(fX-(fW/2)+224, fY-(fH/2)+48+24+32-2, fX+(fW/2)-24, fY-(fH/2)+48+24+32, Utilities.reAlpha(ColorHelper.GREY.getColorCode(), 0.5f*opacity));
				GuiHelper.drawStringFromTTF(FontHandler.ROUNDED_BOLD.get(16), fX-(fW/2)+200+24, fY-(fH/2)+48+24+5, "Starred", Utilities.reAlpha(KiLO.getKiLO().getColorSchemeHandler().getCurrentForeground(), 1f*opacity), Align.LEFT, Align.CENTER);
				GuiHelper.startClip(fX-(fW/2)+224, fY-(fH/2)+48+24+32, fX+(fW/2)-24+ssl.sbw, fY+(fH/2)-48-24);
				ssl.render(opacity);
				GuiHelper.endClip();
			} else {
