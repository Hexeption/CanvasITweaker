...
public class UIMainMenu extends UI {
	...
	public void init() {
		...
		interactables.add(new Link(this, "Go to MyKiLO", 110, Display.getHeight() - 94 + (32) + (FontHandler.ROUNDED_BOLD.get(12).getHeight() /  2), FontHandler.ROUNDED_LIGHT.get(12), 0xFFFFFFFF, Align.LEFT, Align.TOP));
		if (KiLO.getKiLO().outOfDate())
	...
	public void handleInteraction(Interactable i) {
		...
		case 8:
			if (KiLO.getKiLO().outOfDate()) {
				Utilities.openWeb(KiLO.getKiLO().getUserControl().currentVersionURL);
			}
			break;
		}
	}
	...
	public void render(float opacity) {
		...
		GuiHelper.drawTexturedRectangle(x, y, w, h, logo, Utilities.reAlpha(KiLO.getKiLO().getColorSchemeHandler().getCurrentForeground(), 1F  *  opacity));
		...
		String nametag = KiLO.getKiLO().getUserControl().ircTag;
		boolean premium = KiLO.getKiLO().getUserControl().isPremium;
		...
		String[] accountInfo = new String[] {nametag + " " + name, friends.size() + " Friends (" + friendsOnline.size() + " Online)", "Go to MyKiLO"};
		float maxW = 0;
		...
		if (KiLO.getKiLO().getUserControl().kiloHead.getTexture() != null) {
			GuiHelper.drawTexturedRectangle(x, y, w, h, KiLO.getKiLO().getUserControl().kiloHead.getTexture(), 1f * opacity);
		}
