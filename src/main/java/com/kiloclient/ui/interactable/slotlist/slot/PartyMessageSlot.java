...
public class PartyMessageSlot extends Slot {
	...
	public void render(float opacity) {
		...
		if (head != null) {
			if (!this.message.user.equalsIgnoreCase(KiLO.getKiLO().getUserControl().minecraftName))
		...
		if (!this.message.user.equalsIgnoreCase(KiLO.getKiLO().getUserControl().minecraftName) && message.message != "") {
			for (String s : lines) {
			...
			for (String s : lines) {
				...
				if (oneLine)
					GuiHelper.drawRoundedRectangle(this.x + xOffset - 50, yC - 15, this.x + xOffset - 50 + FontHandler.ROUNDED.get(14).getWidth(s) + 20, this.y - 20 + this.height, KiLO.getKiLO().getColorSchemeHandler().getCurrentBackground());
				else
					GuiHelper.drawRoundedRectangle(this.x + xOffset - 50 + lineOffset - 10, yC - 15, this.x + xOffset - 50 + FontHandler.ROUNDED.get(14).getWidth(biggestLine) + 10 + lineOffset, this.y + lines.size() * 24 - 5, KiLO.getKiLO().getColorSchemeHandler().getCurrentBackground());
				if (oneLine) {
