...
public class ActivitySlotNewMessageGroup extends ActivitySlot {
	...
	public ActivitySlotNewMessageGroup(SlotList p, int i, float x, float y, float w, float h, float ox, float oy) {
	...
	public void doubleClick(int mx, int my) {
		...
		for (Party p : KiLO.getKiLO().getPartyManager().getParties()) {
			if (Integer.toString(p.partyID) == ((Activity)ActivityManager.getActivity(index)).partyID)
				party = p;
		}
		
		KiLO.getKiLO().getUIHandler().changeUI(new UIInGameMenuPartyChat(new UIInGameMenu(null, false), party));
		new Thread() {
			@Override
			public void run() {
