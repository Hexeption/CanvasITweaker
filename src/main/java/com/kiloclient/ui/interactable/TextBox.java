...
public class TextBox extends Interactable {
	...
	public TextBox(InteractableParent p, String t, float x, float y, float w, float h, TrueTypeFont f, int fc, Align fh, Align fv) {
		super(p, Interactable.InteractableType.TEXT_BOX, x, y, w, h, f, fc, fh, fv);
		placeholder = t;
		text = "";
	}
	...
	public void update(int mx, int my) {
		...
		if (!active) {
			...
			if (dragging) {
				cursorPos = ChatUtilities.getMousePos(text, font, x, mx, scroll);
			}
		}
		...
		if (active) {
			...
			case Keyboard.KEY_BACK:
				if (text.length() > 0) {
					if (startSelect != endSelect) {
						text = ChatUtilities.replaceAt(text, "", startSelect, endSelect);
						...
						if (Keyboard.isKeyDown(Keyboard.KEY_LCONTROL) || Keyboard.isKeyDown(Keyboard.KEY_RCONTROL)) {
							...
							if (next != -1) {
								text = ChatUtilities.replaceAt(text, "", next, cursorPos);
								cursorPos = next;
							}
						} else {
							text = ChatUtilities.replaceAt(text, "", cursorPos-1, cursorPos);
							cursorPos--;
						}
					}
				}
				...
				if (text.length() > 0) {
					if (startSelect != endSelect) {
						...
						if (Keyboard.isKeyDown(Keyboard.KEY_LCONTROL) || Keyboard.isKeyDown(Keyboard.KEY_RCONTROL)) {
							...
							if (next != -1) {
								text = ChatUtilities.replaceAt(text, "", cursorPos, next);
							}
						} else {
							text = ChatUtilities.replaceAt(text, "", cursorPos, cursorPos+1);
						}
					}
				}
				startSelect = cursorPos;
			...
			case Keyboard.KEY_V:
				if (Keyboard.isKeyDown(Keyboard.KEY_LCONTROL) || Keyboard.isKeyDown(Keyboard.KEY_RCONTROL)) {
					...
					if (cb != "") {
						if (startSelect != endSelect) {
							text = ChatUtilities.replaceAt(text, cb, startSelect, endSelect);
							if (cursorPos > startSelect) {
								cursorPos-= Math.max(startSelect, endSelect)-Math.min(startSelect, endSelect);
							}
							cursorPos+= cb.length();
							...
							text = ChatUtilities.insertAt(text, cb, cursorPos);
							cursorPos+= cb.length();
							startSelect = cursorPos;
						}
					} 
	...
	public void keyTyped(int key, char keyChar) {
		if (active) {
			if (ChatUtilities.isAllowedCharacter(keyChar)) {
				if (startSelect != endSelect) {
					text = ChatUtilities.replaceAt(text, Character.toString(keyChar), startSelect, endSelect);
				} else {
					text = ChatUtilities.insertAt(text, Character.toString(keyChar), cursorPos);
				}
				cursorPos++;
				startSelect = cursorPos;
			}
	...
	public void render(float opacity) {
		...
		GuiHelper.drawRectangle(startX, y+(height/2)-(font.getHeight(text)/2), endX, y+(height/2)+(font.getHeight(text)/2), Utilities.reAlpha(ColorHelper.DARK_BLUE.getColorCode(), 0.5f*opacity));
		
		GuiHelper.drawStringFromTTF(font, x+scroll, y+(height/2), text.length() != 0 || active?text:placeholder, Utilities.reAlpha(fontColor, ((anim/2)+0.5f)*opacity), fontAlignH, fontAlignV);
